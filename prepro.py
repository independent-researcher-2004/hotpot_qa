...
def process_file(filename, config, word_counter=None, char_counter=None):
    ...
    outputs = Parallel(n_jobs=-1, verbose=10)(
        delayed(_process_article)(article, config) for article in data
    )
    print(outputs)
...
def build_features(config, examples, data_type, out_file, word2idx_dict, char2idx_dict):
    ...
    for example in tqdm(examples):
        ...
        context_idxs[: len(example["context_tokens"])] = [
            _get_word(token) for token in example["context_tokens"]
        ]
        ques_idxs[: len(example["ques_tokens"])] = [
            _get_word(token) for token in example["ques_tokens"]
        ]
...
def prepro(config):
    ...
    if config.data_split == "train":
        ...
        examples, eval_examples = process_file(
            config.data_file, config, word_counter, char_counter
        )
    else:
    ...
    if os.path.isfile(config.word2idx_file):
        ...
        word_emb_mat, word2idx_dict, idx2word_dict = get_embedding(
            word_counter,
            "word",
    ...
    build_features(
        config, examples, config.data_split, record_file, word2idx_dict, char2idx_dict
    )
